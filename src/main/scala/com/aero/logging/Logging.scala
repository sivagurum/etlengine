...
trait Logging {
  ...
  protected def logComponentName(msg: => String) {
    if (log.isInfoEnabled) log.info(s"___COMPONENET TYPE___: $msg")
  }
  
  protected def logOutputName(msg: => String) {
    if (log.isInfoEnabled) log.info(s"___OUTPUT DATAFRAME NAME___: $msg")
  }
  ...
  private def initializeLogging(isInterpreter: Boolean, silent: Boolean): Unit = {
    ...
    if (Logging.isLog4j12()) {
      ...
      if (!log4j12Initialized) {
        ...
        val defaultLogProps = """com/aero/core/log4j.properties"""
