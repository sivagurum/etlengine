...
object Reformat{
   ...
   def reformatDataFrame(inputDF: DataFrame, reformatCol:List[(String, String)], otherOptions: Map[String,String]): DataFrame = {
     ...
     if (otherOptions.getOrElse("limit", "N") == "N") outputDF else outputDF.limit(otherOptions.getOrElse("limit", "50").toInt)
   }
   
   val dfImdb = spark.read.format("csv").option("header", "true").load("/Users/jagadeeshkumarsellappan/imdb_1000.csv")
   val reformatDataFrameFn = reformatDataFrame _
