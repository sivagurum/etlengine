...
//       dfNull.show
     ...
     val sch = StructType(Array(StructField("star_rating",DoubleType,true), 
   ...
   def sequenceDataFrame(inputDF: DataFrame, startVal: Int, stepVal: Int, targetCol: String,colOrd: String*): DataFrame = {
     ...
     val partitionDataFrameFn = partitionDataFrame _
...
//     sch.foreach(println)
     //println(sch)
     //val dfPart = partitionDataFrameFn(dfImdb,List("genre"), 5)
     //dfPart.show(100)
     //val dfPartSort = dfPart.sortWithinPartitions("genre")
     //dfPartSort.show(100)
     //println("dddddd ->" + dfPart.rdd.getNumPartitions)
     
     val selColsFisrt = dfImdb.columns.map{x => "first(" + x + ") as " + x}
     val selColsLast = dfImdb.columns.map{x => "last(" + x + ") as " + x}
     
     dfImdb.selectExpr(selColsFisrt: _*).show
     dfImdb.selectExpr(selColsLast: _*).show
     
     //val dfImdbhead = dfImdb.limit(1)
     //dfImdbhead.show
     //val dfImdbMinusHead = dfImdb.except(dfImdbhead)
     //val seqDF = sequenceDataFrameFn01(dfPart,100,-1,"seqNumNew")
     //seqDF.explain
     //seqDF.show
     //seqDF.write.csv("/Users/jagadeeshkumarsellappan/imdb_1000_out4.csv")
